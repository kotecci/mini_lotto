...
def add_combination_id
  Result.all(:conditions => 'combination_id IS NULL AND no > 2000').each do |r|
...
def parse_results
    ...
    arr = arr + numbers
		...
		raise "Takie losowanie istnieje w bazie" unless Result.find_by_no(arr[0]).nil?
      ...
      :set_30_39 => stats[:set_30_39],
      :set_40_42 => stats[:set_40_42],
      :set_1_14 => stats[:set_1_14],
      :set_15_28 => stats[:set_15_28],
      :set_29_42 => stats[:set_29_42],
      :set_a => stats[:set_a],
      :set_b => stats[:set_b],
      :set_c => stats[:set_c],
      :set_d => stats[:set_d],
      ...
      :set_y => stats[:set_y],
      :set_z => stats[:set_z],
      :combination_id => Combination.find_by_n1_and_n2_and_n3_and_n4_and_n5(combination[0], combination[1], combination[2], combination[3], combination[4]).id
	  )
...
def add_rare	
	Result.all(:conditions => 'rares.result_id IS NULL and results.no > 2500', :include => :rare).each do |r|
		rare = Rare.new
		(1..42).each do |rr|
...
def most_often	
	Result.all(:conditions => 'most_oftens.result_id IS NULL and results.no > 2500', :include => :most_often).each do |r|
		mo = MostOften.new
		(1..42).each do |rr|
...
def most_brak
	Result.all(:conditions => 'most_breaks.result_id IS NULL AND results.no > 2500', :include => :most_break).each do |r|
		mo = MostBreak.new
		(1..42).each do |rr|
      puts "a"
...
def add_combination_new_sets
	...
	Combination.all(:conditions => 'set_a IS NULL and id < 900000').each do |r|
		puts r.id
		stats = {
      :set_a => 0, :set_b => 0, :set_c  => 0,	:set_d  => 0,
      :set_s  => 0, :set_t  => 0,	:set_u  => 0, :set_w  => 0,
...
def act
  ...
  Result.all(:conditions => 'set_1_14 IS NULL').each do |r|
    puts r.no
    stats = {
      :set_a => 0, :set_b => 0, :set_c  => 0,	:set_d  => 0,
      :set_s  => 0, :set_t  => 0,	:set_u  => 0, :set_w  => 0,
...
def act_2
  ...
  Combination.all(:conditions => 'id <860000 and set_1_14 is null').each do |r|
    puts r.id
    stats = {
